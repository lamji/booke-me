                ...
                >
                    <div className="text-center pb-2 pt-8 px-6 border-b border-border/10">
                        <h1 className="text-2xl font-bold">
                            {vm.showSummary ? "Confirm Your Request" : "Book Your Event"}
                        </h1>
                        <p className="text-muted-foreground text-sm mt-1">
                            {vm.showSummary
                                ? "Please review your booking details and price before confirming."
                    ...
                    <div className="p-6 space-y-6">
                            ...
                            >
                                <div className="p-4 rounded-xl bg-muted/10 border border-border/40 space-y-4">
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                                        ...
                                        <p className="font-semibold text-right">{vm.clientName}</p>
                                        <p className="text-muted-foreground">Event Type</p>
                                        <p className="font-semibold text-right">{vm.eventType}</p>
                                        <p className="text-muted-foreground">Date & Time</p>
                                        <p className="font-semibold text-right">
                                            {vm.eventDate ? format(vm.eventDate, "MMM dd, yyyy") : ""} at {vm.eventTime}
                                        </p>
                                        <p className="text-muted-foreground">Contact</p>
                                        <p className="font-semibold text-right">{vm.clientPhone}</p>
                                    </div>

                                    {vm.addOns.length > 0 && (
                                        <div className="pt-3 border-t border-border/10">
                                            <p className="text-xs uppercase font-bold text-muted-foreground tracking-widest mb-2">Selected Add-ons</p>
                                            <div className="space-y-1">
                                                {vm.addOns.map(name => (
                                                    <div key={name} className="flex justify-between text-sm">
                                                        <span>{name}</span>
                                                        <span className="text-muted-foreground">+₱{vm.currentEvent?.addons?.find(a => a.name === name)?.price.toLocaleString()}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    <div className="pt-3 border-t border-border/10 flex justify-between items-center">
                                        <span className="text-lg font-bold">Total Est. Price</span>
                                        <span className="text-xl font-black text-orange-600">
                                            {formatPHP(vm.calculateTotalPrice())}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <Button
                                    variant="ghost"
                                    onClick={() => vm.setShowSummary(false)}
                                    className="w-full text-xs uppercase font-bold text-muted-foreground tracking-tighter"
                                >
                                    ← Edit Details
                                </Button>
                            </motion.div>
                        ...
                        {/* Submit */}
                        <Button
                            onClick={vm.showSummary ? vm.submitBooking : vm.handleSummary}
